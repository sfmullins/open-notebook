...
from open_notebook.database.repository import repo_get, repo_list
...
class Model(ObjectModel):
    ...
    async def get_models_by_type(cls, model_type):
        return [Model(**row) for row in await repo_list("model", filters={"type": model_type})]
    ...
    async def get_display_info_for_ids(
        ...
        if not ids:
            return {}
        try:
            result = await repo_list("model", in_filters={"id": ids})
    ...
    async def get_by_credential(cls, credential_id: str):
        return [
            Model(**row)
            for row in await repo_list("model", filters={"credential": credential_id})
        ]

    def _prepare_save_data(self) -> Dict[str, Any]:
        data = super()._prepare_save_data()
        if data.get("credential"):
            data["credential"] = str(data["credential"])
        return data
...
class ModelManager:
    ...
    async def get_model(self, model_id: str, **kwargs) -> Optional[ModelType]:
        ...
        if model.credential:
            ...
            if credential:
                ...
                logger.warning(
                    f"Model {model.id} has credential {model.credential} but it could not be loaded. Falling back to env vars."
                )
        ...
        if model.type == "language":
            return AIFactory.create_language(model_name=model.name, provider=provider, config=config)
        if model.type == "embedding":
            return AIFactory.create_embedding(model_name=model.name, provider=provider, config=config)
        if model.type == "speech_to_text":
            return AIFactory.create_speech_to_text(model_name=model.name, provider=provider, config=config)
        if model.type == "text_to_speech":
            return AIFactory.create_text_to_speech(model_name=model.name, provider=provider, config=config)
        raise ConfigurationError(f"Invalid model type: {model.type}")
    ...
    async def get_speech_to_text(self, **kwargs) -> Optional[SpeechToTextModel]:
        ...
        assert model is None or isinstance(model, SpeechToTextModel), f"Expected SpeechToTextModel but got {type(model)}"
        return model
    ...
    async def get_text_to_speech(self, **kwargs) -> Optional[TextToSpeechModel]:
        ...
        if not model_id:
            return None
        model = await self.get_model(model_id, **kwargs)
        assert model is None or isinstance(model, TextToSpeechModel), f"Expected TextToSpeechModel but got {type(model)}"
        return model
    ...
    async def get_default_model(self, model_type: str, **kwargs) -> Optional[ModelType]:
        ...
        if not model_id:
            logger.warning(
                f"No default model configured for type '{model_type}'. Please go to Settings → Models and set a default model."
            )
            return None
        try:
            ...
            logger.error(
                f"Failed to load default model for type '{model_type}': {e}. The configured model_id '{model_id}' may have been deleted or misconfigured. Please go to Settings → Models and reconfigure the default model."
            )
            return None
